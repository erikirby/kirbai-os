...
const PLATFORM_ICONS: Record<string, any> = {
    ...
    tiktok: <div className="w-4 h-4 rounded-full bg-black flex items-center justify-center text-[8px] font-black text-foreground outline outline-cyan-500/50 outline-offset-[1px] shadow-[1px_1px_0_theme(colors.pink.500)]">t</div>,
...
export default function CompetitorTracker({ theme, mode = 'kirbai' }: { theme?: string; mode?: string }) {
    ...
    return (
        <div className="w-full flex flex-col gap-8 mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {/* New Competitor Form */}
            {showNew && (
                <div className="card p-6 flex flex-col gap-4 animate-in slide-in-from-top-4">
                    <h2 className="section-eyebrow mb-2">Identify Target</h2>
                    ...
                    <div className="flex gap-4">
                        <input
                            ...
                            className="flex-1 px-4 py-3 bg-surface/40 border border-border rounded-xl text-foreground font-bold focus:outline-none focus:border-accent/40 placeholder:text-foreground/20"
                        />
                        <select
                            value={form.platform}
                    ...
                    <div className="flex gap-4 items-center">
                        <div className="flex-1 flex items-center bg-surface/40 border border-border rounded-xl overflow-hidden focus-within:border-accent/40">
                            <div className="pl-4 pr-2 text-foreground/20"><LinkIcon className="w-4 h-4" /></div>
                            <input
                                value={form.handleUrl}
                                onChange={e => setForm(f => ({ ...f, handleUrl: e.target.value }))}
                                placeholder="https://youtube.com/@rival... or @rival_handle"
                                className="w-full px-2 py-3 bg-transparent text-foreground focus:outline-none placeholder:text-foreground/20 text-sm font-mono"
                            />
                        </div>
                    </div>
                    ...
                    <textarea
                        ...
                        rows={3}
                        className="w-full px-4 py-3 bg-surface/40 border border-border rounded-xl text-foreground/80 text-sm focus:outline-none focus:border-accent/40 resize-none placeholder:text-foreground/20"
                    />
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {competitors.map(comp => (
                        <div key={comp.id} className="card p-5 flex flex-col gap-3 group relative overflow-hidden">
                            ...
                            <div className="flex items-start justify-between">
                                <div className="flex items-center gap-2">
                                    <div className="w-8 h-8 rounded-lg bg-surface/40 flex items-center justify-center border border-border shadow-inner">
                            ...
                            {comp.notes && (
                                <p className="text-sm text-foreground/60 mt-2 bg-surface/20 p-3 rounded-xl border border-border leading-relaxed line-clamp-3">
                                    {comp.notes}
                                </p>
                            )}
                        </div>

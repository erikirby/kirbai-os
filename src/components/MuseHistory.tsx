        ...
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-right-10 duration-500">
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xl font-semibold">History Vault</h3>
                </div>
                <button 
                    onClick={onBack}
                    className="flex items-center gap-2 btn-secondary rounded-full px-4 py-2 text-xs font-semibold hover:bg-white/10 transition-all"
                >
            ...
            {cards.length === 0 ? (
                <div className="card p-12 text-center opacity-40">
                    <p className="text-xs font-semibold">No accepted proposals yet.</p>
                </div>
            ) : (
                    ...
                    {cards.map((card) => (
                        <div key={card.id} className="card p-6 hover:border-accent/40 transition-all group">
                            <div className="flex justify-between items-start mb-2">
                                <span className="text-[11px] font-semibold text-accent/60">{card.type}</span>
                            ...
                            <div className="flex gap-2">
                                {Object.entries(card.actionMatrix).map(([k, v]) => (
                                    <span key={k} className="text-[11px] font-semibold tracking-tighter px-2 py-0.5 bg-black/40 rounded-md border border-white/5 text-foreground/30">

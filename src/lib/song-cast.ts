// Song -> cast data (persistence key `song_cast`). Shared by the Cast Sheet API and the AI ops.
import { getRow, setRow } from '@/lib/db';

export const SONG_CAST_KEY = 'song_cast';
...
export function seedSongCast(): SongCast {
    const eras = [
        'Next Era',
        'Singles',
        'Pink Rare Candies',
...
export async function loadSongCast(): Promise<SongCast> {
    let data = await getRow(SONG_CAST_KEY) as SongCast | null;
    if (!data) {
        data = seedSongCast();
        await setRow(SONG_CAST_KEY, data);
    }
    return data;
}

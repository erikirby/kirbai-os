...
export async function POST(req: NextRequest) {
    try {
        ...
        if (directorResult.usageMetadata) {
            await logApiUsageAsync("/api/director/plan (Director)", directorResult.usageMetadata.promptTokenCount || 0, directorResult.usageMetadata.candidatesTokenCount || 0);
        }
        ...
        if (strategistResult.usageMetadata) await logApiUsageAsync("/api/director/plan (Strategist)", strategistResult.usageMetadata.promptTokenCount || 0, strategistResult.usageMetadata.candidatesTokenCount || 0);
        if (audienceResult.usageMetadata) await logApiUsageAsync("/api/director/plan (Audience)", audienceResult.usageMetadata.promptTokenCount || 0, audienceResult.usageMetadata.candidatesTokenCount || 0);
        ...
        if (refinedResult.usageMetadata) {
            await logApiUsageAsync("/api/director/plan (Refinement)", refinedResult.usageMetadata.promptTokenCount || 0, refinedResult.usageMetadata.candidatesTokenCount || 0);
        }
